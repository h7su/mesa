...
visit_load_global(isel_context* ctx, nir_intrinsic_instr* instr)
{
   ...
   info.cache = get_cache_flags(ctx, nir_intrinsic_access(instr) | ACCESS_TYPE_LOAD);
   ...
   if (info.dst.type() == RegType::vgpr ||
       (ctx->options->gfx_level < GFX8 && info.cache.value & ac_glc) || !can_use_smem) {

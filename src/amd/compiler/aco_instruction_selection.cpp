                   ...
                   unsigned align, unsigned const_offset, Temp dst_hint)
{
   ...
   load->smem().cache = get_load_cache_flags(info.glc, false);
                    ...
                    unsigned align_, unsigned const_offset, Temp dst_hint)
{
   ...
   mubuf->mubuf().cache = get_load_cache_flags(info.glc, info.slc);
                     ...
                     unsigned align_, unsigned const_offset, Temp dst_hint)
{
   ...
   if (use_mubuf) {
      ...
      mubuf->mubuf().cache = get_load_cache_flags(info.glc, false);
      mubuf->mubuf().offset = const_offset;
      ...
      flat->flatlike().cache = get_load_cache_flags(info.glc, false);
                    ...
                    unsigned alignment, unsigned const_offset, Temp dst_hint)
{
   ...
   mtbuf->mtbuf().cache = get_load_cache_flags(info.glc, info.slc);
...
visit_image_load(isel_context* ctx, nir_intrinsic_instr* instr)
{
   ...
   if (dim == GLSL_SAMPLER_DIM_BUF) {
      ...
      load->mubuf().cache = get_load_cache_flags(glc, false);
      ...
      load->cache = get_load_cache_flags(glc, false);
...
visit_image_store(isel_context* ctx, nir_intrinsic_instr* instr)
{
   ...
   if (dim == GLSL_SAMPLER_DIM_BUF) {
      ...
      store->mubuf().cache = get_store_cache_flags(glc, false);
   ...
   store->cache = get_store_cache_flags(glc, false);
...
visit_image_atomic(isel_context* ctx, nir_intrinsic_instr* instr)
{
   ...
   if (dim == GLSL_SAMPLER_DIM_BUF) {
      ...
      mubuf->mubuf().cache = get_atomic_cache_flags(return_previous);
   ...
   mimg->cache = get_atomic_cache_flags(return_previous);
...
visit_store_ssbo(isel_context* ctx, nir_intrinsic_instr* instr)
{
   ...
   for (unsigned i = 0; i < write_count; i++) {
      ...
      store->mubuf().cache = get_store_cache_flags(
         glc || (ctx->program->gfx_level == GFX6 && write_datas[i].bytes() < 4), false);
      store->mubuf().disable_wqm = true;
      store->mubuf().sync = sync;
      ctx->program->needs_exact = true;
      ctx->block->instructions.emplace_back(std::move(store));
...
visit_store_global(isel_context* ctx, nir_intrinsic_instr* instr)
{
   ...
   for (unsigned i = 0; i < write_count; i++) {
      ...
      if (ctx->options->gfx_level >= GFX7) {
         ...
         flat->flatlike().cache = get_store_cache_flags(glc, false);
         ...
         mubuf->mubuf().cache = get_store_cache_flags(glc || write_datas[i].bytes() < 4, false);
...
visit_global_atomic(isel_context* ctx, nir_intrinsic_instr* instr)
{
   ...
   if (ctx->options->gfx_level >= GFX7) {
      ...
      flat->flatlike().cache = get_atomic_cache_flags(return_previous);
      assert(global || !const_offset);
      flat->flatlike().offset = const_offset;
      flat->flatlike().disable_wqm = true;
...
visit_store_buffer(isel_context* ctx, nir_intrinsic_instr* intrin)
{
   ...
   for (unsigned i = 0; i < write_count; i++) {
      ...
      ac_hw_cache_flags cache = get_store_cache_flags(glc, slc);

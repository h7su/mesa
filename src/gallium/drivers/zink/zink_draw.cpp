...
zink_emit_stream_output_targets(struct pipe_context *pctx)
{
   ...
   VKCTX(CmdBindTransformFeedbackBuffersEXT)(batch->state->main_cmdbuf.vk, 0, ctx->num_so_targets,
...
{
   VKCTX(CmdPushConstants)(ctx->batch.state->main_cmdbuf.vk, ctx->curr_program->base.layout, VK_SHADER_STAGE_ALL_GRAPHICS,
...
draw_indexed_need_index_buffer_unref(struct zink_context *ctx,
             ...
             bool needs_drawid)
{
   struct zink_cmdbuf *cmdbuf = &ctx->batch.state->main_cmdbuf;
   if (dinfo->increment_draw_id && needs_drawid) {
      for (unsigned i = 0; i < num_draws; i++) {
             ...
             bool needs_drawid)
{
   ...
   if (dinfo->increment_draw_id && needs_drawid) {
      ...
      if (needs_drawid)
         update_drawid(ctx, draw_id);
      if (HAS_MULTIDRAW) {
         VKCTX(CmdDrawMultiIndexedEXT)(cmdbuf->vk, num_draws, (const VkMultiDrawIndexedInfoEXT*)draws,
   ...
   if (pipeline) {
      ...
      if (BATCH_CHANGED || pipeline_changed || ctx->shobj_draw) {
         ...
         if (unlikely(can_dgc && screen->info.nv_dgc_props.maxGraphicsShaderGroupCount == 1)) {
            ...
            VKCTX(CmdBindPipeline)(bs->main_cmdbuf.vk, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline);
         }
      }
      ...
      if (BATCH_CHANGED || shaders_changed || !ctx->shobj_draw) {
         ...
         VKCTX(CmdBindShadersEXT)(bs->main_cmdbuf.vk, ZINK_GFX_SHADER_COUNT, stages, ctx->curr_program->objects);
         VKCTX(CmdSetDepthBiasEnable)(bs->main_cmdbuf.vk, VK_TRUE);
         VKCTX(CmdSetTessellationDomainOriginEXT)(bs->main_cmdbuf.vk, VK_TESSELLATION_DOMAIN_ORIGIN_LOWER_LEFT);
         VKCTX(CmdSetSampleLocationsEnableEXT)(bs->main_cmdbuf.vk, ctx->gfx_pipeline_state.sample_locations_enabled);
         VKCTX(CmdSetRasterizationStreamEXT)(bs->main_cmdbuf.vk, 0);
      }
      ctx->shobj_draw = true;
   }
          ...
          uint32_t partial_velem_mask)
{
   ...
   if (unlikely(zink_debug & ZINK_DEBUG_SYNC)) {
      ...
      VKSCR(CmdPipelineBarrier)(ctx->batch.state->main_cmdbuf.vk,
   ...
   if (index_size) {
      ...
      if (unlikely(can_dgc)) {
         ...
         VKCTX(CmdBindIndexBuffer)(batch->state->main_cmdbuf.vk, res->obj->buffer, index_offset, index_type[index_size >> 1]);
      }
   }
   ...
   if (BATCH_CHANGED || ctx->vp_state_changed || (DYNAMIC_STATE == ZINK_NO_DYNAMIC_STATE && pipeline_changed)) {
      ...
      if (DYNAMIC_STATE != ZINK_NO_DYNAMIC_STATE)
         VKCTX(CmdSetViewportWithCount)(batch->state->main_cmdbuf.vk, ctx->vp_state.num_viewports, viewports);
      else
         VKCTX(CmdSetViewport)(batch->state->main_cmdbuf.vk, 0, ctx->vp_state.num_viewports, viewports);
   }
   if (BATCH_CHANGED || ctx->scissor_changed || ctx->vp_state_changed || (DYNAMIC_STATE == ZINK_NO_DYNAMIC_STATE && pipeline_changed)) {
      ...
      if (DYNAMIC_STATE != ZINK_NO_DYNAMIC_STATE)
         VKCTX(CmdSetScissorWithCount)(batch->state->main_cmdbuf.vk, ctx->vp_state.num_viewports, scissors);
      else
         VKCTX(CmdSetScissor)(batch->state->main_cmdbuf.vk, 0, ctx->vp_state.num_viewports, scissors);
   }
   ctx->vp_state_changed = false;
   ctx->scissor_changed = false;

   if (BATCH_CHANGED || ctx->stencil_ref_changed) {
      VKCTX(CmdSetStencilReference)(batch->state->main_cmdbuf.vk, VK_STENCIL_FACE_FRONT_BIT,
                               ctx->stencil_ref.ref_value[0]);
      VKCTX(CmdSetStencilReference)(batch->state->main_cmdbuf.vk, VK_STENCIL_FACE_BACK_BIT,
                               ctx->stencil_ref.ref_value[1]);
      ctx->stencil_ref_changed = false;
   }

   if (DYNAMIC_STATE != ZINK_NO_DYNAMIC_STATE && (BATCH_CHANGED || ctx->dsa_state_changed)) {
      VKCTX(CmdSetDepthBoundsTestEnable)(batch->state->main_cmdbuf.vk, dsa_state->hw_state.depth_bounds_test);
      if (dsa_state->hw_state.depth_bounds_test)
         VKCTX(CmdSetDepthBounds)(batch->state->main_cmdbuf.vk,
                             dsa_state->hw_state.min_depth_bounds,
                             dsa_state->hw_state.max_depth_bounds);
      VKCTX(CmdSetDepthTestEnable)(batch->state->main_cmdbuf.vk, dsa_state->hw_state.depth_test);
      VKCTX(CmdSetDepthCompareOp)(batch->state->main_cmdbuf.vk, dsa_state->hw_state.depth_compare_op);
      VKCTX(CmdSetDepthWriteEnable)(batch->state->main_cmdbuf.vk, dsa_state->hw_state.depth_write);
      VKCTX(CmdSetStencilTestEnable)(batch->state->main_cmdbuf.vk, dsa_state->hw_state.stencil_test);
      if (dsa_state->hw_state.stencil_test) {
         VKCTX(CmdSetStencilOp)(batch->state->main_cmdbuf.vk, VK_STENCIL_FACE_FRONT_BIT,
         ...
         VKCTX(CmdSetStencilOp)(batch->state->main_cmdbuf.vk, VK_STENCIL_FACE_BACK_BIT,
         ...
         if (dsa_state->base.stencil[1].enabled) {
            VKCTX(CmdSetStencilWriteMask)(batch->state->main_cmdbuf.vk, VK_STENCIL_FACE_FRONT_BIT, dsa_state->hw_state.stencil_front.writeMask);
            VKCTX(CmdSetStencilWriteMask)(batch->state->main_cmdbuf.vk, VK_STENCIL_FACE_BACK_BIT, dsa_state->hw_state.stencil_back.writeMask);
            VKCTX(CmdSetStencilCompareMask)(batch->state->main_cmdbuf.vk, VK_STENCIL_FACE_FRONT_BIT, dsa_state->hw_state.stencil_front.compareMask);
            VKCTX(CmdSetStencilCompareMask)(batch->state->main_cmdbuf.vk, VK_STENCIL_FACE_BACK_BIT, dsa_state->hw_state.stencil_back.compareMask);
         } else {
            VKCTX(CmdSetStencilWriteMask)(batch->state->main_cmdbuf.vk, VK_STENCIL_FACE_FRONT_AND_BACK, dsa_state->hw_state.stencil_front.writeMask);
            VKCTX(CmdSetStencilCompareMask)(batch->state->main_cmdbuf.vk, VK_STENCIL_FACE_FRONT_AND_BACK, dsa_state->hw_state.stencil_front.compareMask);
         }
      } else {
         VKCTX(CmdSetStencilWriteMask)(batch->state->main_cmdbuf.vk, VK_STENCIL_FACE_FRONT_AND_BACK, dsa_state->hw_state.stencil_front.writeMask);
         VKCTX(CmdSetStencilCompareMask)(batch->state->main_cmdbuf.vk, VK_STENCIL_FACE_FRONT_AND_BACK, dsa_state->hw_state.stencil_front.compareMask);
         VKCTX(CmdSetStencilOp)(batch->state->main_cmdbuf.vk, VK_STENCIL_FACE_FRONT_AND_BACK, VK_STENCIL_OP_KEEP, VK_STENCIL_OP_KEEP, VK_STENCIL_OP_KEEP, VK_COMPARE_OP_ALWAYS);
      }
   }
   ...
   if (BATCH_CHANGED || rast_state_changed) {
      if (DYNAMIC_STATE != ZINK_NO_DYNAMIC_STATE) {
         VKCTX(CmdSetFrontFace)(batch->state->main_cmdbuf.vk, (VkFrontFace)ctx->gfx_pipeline_state.dyn_state1.front_face);
         VKCTX(CmdSetCullMode)(batch->state->main_cmdbuf.vk, ctx->gfx_pipeline_state.dyn_state1.cull_mode);
      }

      if (DYNAMIC_STATE >= ZINK_DYNAMIC_STATE3) {
         if (ctx->ds3_states & BITFIELD_BIT(ZINK_DS3_RAST_STIPPLE))
            VKCTX(CmdSetLineStippleEXT)(batch->state->main_cmdbuf.vk, rast_state->base.line_stipple_factor, rast_state->base.line_stipple_pattern);
         if (ctx->ds3_states & BITFIELD_BIT(ZINK_DS3_RAST_CLIP))
            VKCTX(CmdSetDepthClipEnableEXT)(batch->state->main_cmdbuf.vk, rast_state->hw_state.depth_clip);
         if (ctx->ds3_states & BITFIELD_BIT(ZINK_DS3_RAST_CLAMP))
            VKCTX(CmdSetDepthClampEnableEXT)(batch->state->main_cmdbuf.vk, rast_state->hw_state.depth_clamp);
         if (ctx->ds3_states & BITFIELD_BIT(ZINK_DS3_RAST_POLYGON))
            VKCTX(CmdSetPolygonModeEXT)(batch->state->main_cmdbuf.vk, (VkPolygonMode)rast_state->hw_state.polygon_mode);
         if (ctx->ds3_states & BITFIELD_BIT(ZINK_DS3_RAST_HALFZ))
            VKCTX(CmdSetDepthClipNegativeOneToOneEXT)(batch->state->main_cmdbuf.vk, !rast_state->hw_state.clip_halfz);
         if (ctx->ds3_states & BITFIELD_BIT(ZINK_DS3_RAST_PV))
            VKCTX(CmdSetProvokingVertexModeEXT)(batch->state->main_cmdbuf.vk,
                                                rast_state->hw_state.pv_last ?
                                                VK_PROVOKING_VERTEX_MODE_LAST_VERTEX_EXT :
                                                VK_PROVOKING_VERTEX_MODE_FIRST_VERTEX_EXT);
         if (ctx->ds3_states & BITFIELD_BIT(ZINK_DS3_RAST_CLIP))
            VKCTX(CmdSetLineRasterizationModeEXT)(batch->state->main_cmdbuf.vk, rast_state->dynamic_line_mode);
         if (ctx->ds3_states & BITFIELD_BIT(ZINK_DS3_RAST_STIPPLE_ON))
            VKCTX(CmdSetLineStippleEnableEXT)(batch->state->main_cmdbuf.vk, rast_state->hw_state.line_stipple_enable);
      }
   }
   if ((BATCH_CHANGED || ctx->sample_mask_changed) && screen->have_full_ds3) {
      VKCTX(CmdSetRasterizationSamplesEXT)(batch->state->main_cmdbuf.vk, (VkSampleCountFlagBits)(ctx->gfx_pipeline_state.rast_samples + 1));
      VKCTX(CmdSetSampleMaskEXT)(batch->state->main_cmdbuf.vk, (VkSampleCountFlagBits)(ctx->gfx_pipeline_state.rast_samples + 1), &ctx->gfx_pipeline_state.sample_mask);
   ...
   if ((BATCH_CHANGED || ctx->blend_state_changed)) {
      if (ctx->gfx_pipeline_state.blend_state) {
         if (ctx->ds3_states & BITFIELD_BIT(ZINK_DS3_BLEND_A2C))
            VKCTX(CmdSetAlphaToCoverageEnableEXT)(batch->state->main_cmdbuf.vk, ctx->gfx_pipeline_state.blend_state->alpha_to_coverage &&
         ...
         if (ctx->ds3_states & BITFIELD_BIT(ZINK_DS3_BLEND_A21))
            VKCTX(CmdSetAlphaToOneEnableEXT)(batch->state->main_cmdbuf.vk, ctx->gfx_pipeline_state.blend_state->alpha_to_one);
         if (ctx->fb_state.nr_cbufs) {
            if (ctx->ds3_states & BITFIELD_BIT(ZINK_DS3_BLEND_ON))
               VKCTX(CmdSetColorBlendEnableEXT)(batch->state->main_cmdbuf.vk, 0, ctx->fb_state.nr_cbufs, ctx->gfx_pipeline_state.blend_state->ds3.enables);
            if (ctx->ds3_states & BITFIELD_BIT(ZINK_DS3_BLEND_WRITE))
               VKCTX(CmdSetColorWriteMaskEXT)(batch->state->main_cmdbuf.vk, 0, ctx->fb_state.nr_cbufs, ctx->gfx_pipeline_state.blend_state->ds3.wrmask);
            if (ctx->ds3_states & BITFIELD_BIT(ZINK_DS3_BLEND_EQ))
               VKCTX(CmdSetColorBlendEquationEXT)(batch->state->main_cmdbuf.vk, 0, ctx->fb_state.nr_cbufs, ctx->gfx_pipeline_state.blend_state->ds3.eq);
         }
         if (ctx->ds3_states & BITFIELD_BIT(ZINK_DS3_BLEND_LOGIC_ON))
            VKCTX(CmdSetLogicOpEnableEXT)(batch->state->main_cmdbuf.vk, ctx->gfx_pipeline_state.blend_state->logicop_enable);
         if (ctx->ds3_states & BITFIELD_BIT(ZINK_DS3_BLEND_LOGIC))
            VKCTX(CmdSetLogicOpEXT)(batch->state->main_cmdbuf.vk, ctx->gfx_pipeline_state.blend_state->logicop_func);
      }
   }
   ctx->ds3_states = 0;
   ...
   if (BATCH_CHANGED ||
      ...
      VKCTX(CmdSetLineWidth)(batch->state->main_cmdbuf.vk, rast_state->line_width);
   ...
   if (BATCH_CHANGED || mode_changed ||
      ...
      if (depth_bias) {
         if (rast_state->base.offset_units_unscaled) {
            VKCTX(CmdSetDepthBias)(batch->state->main_cmdbuf.vk, rast_state->offset_units * ctx->depth_bias_scale_factor, rast_state->offset_clamp, rast_state->offset_scale);
         } else {
            VKCTX(CmdSetDepthBias)(batch->state->main_cmdbuf.vk, rast_state->offset_units, rast_state->offset_clamp, rast_state->offset_scale);
         }
      } else {
         VKCTX(CmdSetDepthBias)(batch->state->main_cmdbuf.vk, 0.0f, 0.0f, 0.0f);
      }
   }
   ...
   if (DYNAMIC_STATE != ZINK_NO_DYNAMIC_STATE) {
      if (ctx->sample_locations_changed) {
         ...
         VKCTX(CmdSetSampleLocationsEXT)(batch->state->main_cmdbuf.vk, &loc);
      }
      ctx->sample_locations_changed = false;
   }

   if (BATCH_CHANGED || ctx->blend_color_changed) {
      VKCTX(CmdSetBlendConstants)(batch->state->main_cmdbuf.vk, ctx->blend_constants);
   }
   ...
   if (DYNAMIC_STATE != ZINK_NO_DYNAMIC_STATE && (BATCH_CHANGED || mode_changed))
      VKCTX(CmdSetPrimitiveTopology)(batch->state->main_cmdbuf.vk, zink_primitive_topology(mode));

   if (DYNAMIC_STATE >= ZINK_DYNAMIC_STATE2 && (BATCH_CHANGED || ctx->primitive_restart != dinfo->primitive_restart)) {
      VKCTX(CmdSetPrimitiveRestartEnable)(batch->state->main_cmdbuf.vk, dinfo->primitive_restart);
   ...
   if (DYNAMIC_STATE >= ZINK_DYNAMIC_STATE2 && (BATCH_CHANGED || ctx->rasterizer_discard_changed)) {
      VKCTX(CmdSetRasterizerDiscardEnable)(batch->state->main_cmdbuf.vk, ctx->gfx_pipeline_state.dyn_state2.rasterizer_discard);
   ...
   if (reads_basevertex) {
      ...
      if (unlikely(can_dgc)) {
         ...
         *ptr = draw_mode_is_indexed;
         ...
         VKCTX(CmdPushConstants)(batch->state->main_cmdbuf.vk, ctx->curr_program->base.layout, VK_SHADER_STAGE_ALL_GRAPHICS,
   ...
   if (!screen->optimal_keys) {
      if (zink_get_fs_key(ctx)->lower_line_stipple ||
         ...
         VKCTX(CmdPushConstants)(batch->state->main_cmdbuf.vk,
   ...
   if (have_streamout) {
      ...
      VKCTX(CmdBeginTransformFeedbackEXT)(batch->state->main_cmdbuf.vk, 0, ctx->num_so_targets, counter_buffers, counter_buffer_offsets);
   }
   ...
   if (unlikely(zink_tracing))
      zink_cmd_debug_marker_end(ctx, &batch->state->main_cmdbuf, marker);
   ...
   if (have_streamout) {
      ...
      VKCTX(CmdEndTransformFeedbackEXT)(batch->state->main_cmdbuf.vk, 0, ctx->num_so_targets, counter_buffers, counter_buffer_offsets);
   }
...
zink_vertex_state_mask(struct zink_context *ctx, struct pipe_vertex_state *vstate, uint32_t partial_velem_mask)
{
   ...
   if (partial_velem_mask == vstate->input.full_velem_mask) {
      VKCTX(CmdSetVertexInputEXT)(cmdbuf->vk,
...
zink_bind_vertex_state(struct zink_context *ctx, struct pipe_vertex_state *vstate, uint32_t partial_velem_mask)
{
   ...
   if (unlikely(zink_debug & ZINK_DEBUG_DGC)) {
      ...
      VKCTX(CmdBindVertexBuffers)(cmdbuf->vk, 0,
...
zink_launch_grid(struct pipe_context *pctx, const struct pipe_grid_info *info)
{
   ...
   if (prev_pipeline != pipeline || BATCH_CHANGED)
      VKCTX(CmdBindPipeline)(batch->state->main_cmdbuf.vk, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline);
   if (BATCH_CHANGED) {
